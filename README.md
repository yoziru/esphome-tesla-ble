...
| Method | How | Best for |
|--------|-----|----------|
| **ESPHome Dashboard** | Add the board package, then configure it in the dashboard | HA + ESPHome add-on users |
...
For a new, unflashed board:

1. In the ESPHome Device Builder, select **Create configuration** → **Create new project** and complete the name and Wi-Fi setup.
...
For a board already running this project's firmware, use the Device Builder's **Adopt** action instead. It creates the same device YAML; continue at step 2.

3. Add the vehicle and OTA settings to the top-level `substitutions` section:
```yaml
substitutions:
...
4. Add the board package under the top-level `packages` section. For an M5Stack NanoC6:
...
5. Save and **Install**. Future firmware updates are applied from this same device card with **Update**.

Use `tesla-ble-m5stack-atoms3.dashboard.yml` for an AtomS3 or `tesla-ble-esp32-generic.dashboard.yml` for a generic ESP32.

The first flash requires Wi-Fi, `tesla_vin`, `tesla_ble_mac_address`, and `ota_password`. See [Finding the BLE MAC](#finding-the-ble-mac) if you do not have the MAC yet.

For example, an adopted NanoC6 with the YAML generated by current ESPHome Dashboard should look like this. Preserve the generated name, friendly name, API key, and Wi-Fi secrets from your device.
...
After creating the device and adding your Wi-Fi and VIN secrets:
...
4. Open **Logs** — you'll see:
   ...
   ```
5. Click **Edit** again, **remove** the two lines you added, and add `tesla_ble_mac_address` to your **Secrets**
6. **Install** again
