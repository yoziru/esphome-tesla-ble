...
1. Go to **ESPHome dashboard** → **New Device** and complete the normal Wi-Fi setup. This also applies after adopting a previously flashed device.
2. Open the device's YAML editor. Keep the generated `esphome`, `api`, and `wifi` sections.
3. Add these two values to the top-level `substitutions` section:
...
The first flash requires Wi-Fi, `tesla_vin`, and `tesla_ble_mac_address`. See [Finding the BLE MAC](#finding-the-ble-mac) if you do not have the MAC yet.
