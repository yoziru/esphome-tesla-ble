...
void TeslaBLEVehicle::loadDomainSessionInfo(UniversalMessage_Domain domain) {
  ESP_LOGCONFIG(TAG, "Loading %s session info from NVS..",
                domain_to_string(domain));
  Signatures_SessionInfo session_info = Signatures_SessionInfo_init_default;
  if (nvs_load_session_info(&session_info, domain) != 0) {
    ESP_LOGW(TAG, "Failed to load %s session info from NVS",
             domain_to_string(domain));
  }
}
...
void TeslaBLEVehicle::process_command_queue() {
  ...
  case BLECommandState::IDLE:
    ESP_LOGI(TAG, "[%s] Preparing command..",
  ...
  case BLECommandState::WAITING_FOR_VCSEC_AUTH:
    if (now - current_command.last_tx_at > MAX_LATENCY) {
      auto session = tesla_ble_client_->getPeer(
          UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY);
      if (session->isInitialized()) {
        ESP_LOGD(TAG, "[%s] VCSEC session authenticated",
                 current_command.execute_name.c_str());
        switch (current_command.domain) {
        case UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY:
          current_command.state = BLECommandState::READY;
          break;
        case UniversalMessage_Domain_DOMAIN_INFOTAINMENT:
          ESP_LOGD(TAG, "[%s] Validating INFOTAINMENT session..",
                   current_command.execute_name.c_str());
        ...
        case UniversalMessage_Domain_DOMAIN_BROADCAST:
          ESP_LOGE(
              TAG,
              "[%s] Invalid state: VCSEC authenticated but no auth required",
              current_command.execute_name.c_str());
          // pop command
          command_queue_.pop();
          break;
        ...
        ESP_LOGW(TAG, "[%s] VCSEC auth expired, refreshing session..",
                 current_command.execute_name.c_str());
        ...
        if (current_command.retry_count <= MAX_RETRIES) {
          ...
          ESP_LOGE(
              TAG,
              "[%s] Failed to authenticate VCSEC after %d retries, giving up",
  ...
  case BLECommandState::WAITING_FOR_VCSEC_AUTH_RESPONSE:
    if (now - current_command.last_tx_at > MAX_LATENCY) {
      ESP_LOGW(TAG,
               "[%s] Timeout while waiting for VCSEC SessionInfo, retrying..",
               current_command.execute_name.c_str());
      current_command.state = BLECommandState::WAITING_FOR_VCSEC_AUTH;
    }
    break;
  ...
  case BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH:
    if (now - current_command.last_tx_at > MAX_LATENCY) {
      if (!this->isAsleepSensor->state == false) {
        ESP_LOGW(TAG, "[%s] Car is asleep, initiating wake..",
                 current_command.execute_name.c_str());
        ...
        if (session->isInitialized()) {
          ESP_LOGD(TAG, "[%s] INFOTAINMENT authenticated",
                   current_command.execute_name.c_str());
          current_command.state = BLECommandState::READY;
        } else {
          ESP_LOGW(TAG, "[%s] INFOTAINMENT auth expired, refreshing session..",
                   current_command.execute_name.c_str());
          current_command.retry_count++;
          ...
          if (current_command.retry_count <= MAX_RETRIES) {
            ...
            ESP_LOGE(
                TAG,
                "[%s] Failed INFOTAINMENT auth after %d retries, giving up",
                current_command.execute_name.c_str(), MAX_RETRIES);
            // pop command
            command_queue_.pop();
          }
    ...
    if (now - current_command.last_tx_at > MAX_LATENCY) {
      if (current_command.retry_count > MAX_RETRIES) {
        ...
        ESP_LOGD(TAG, "[%s] Sending wake command | attempt %d/%d",
                 current_command.execute_name.c_str(),
                 current_command.retry_count, MAX_RETRIES);
        int result = this->sendVCSECActionMessage(
            VCSEC_RKEAction_E_RKE_ACTION_WAKE_VEHICLE);
        if (result != 0) {
          ESP_LOGE(TAG, "[%s] Failed to send wake command",
                   current_command.execute_name.c_str());
        }
        current_command.last_tx_at = now;
        current_command.retry_count++;
    ...
    if (now - current_command.last_tx_at > MAX_LATENCY) {
      if (this->isAsleepSensor->state == false) {
        if (strcmp(current_command.execute_name.c_str(), "wake vehicle") == 0) {
          ESP_LOGI(TAG, "[%s] Vehicle is awake, command completed",
                   current_command.execute_name.c_str());
          command_queue_.pop();
        } else {
          ESP_LOGI(TAG, "[%s] Vehicle is awake, waiting for infotainment auth",
                   current_command.execute_name.c_str());
          current_command.state =
              BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH;
        ...
        if (current_command.retry_count % 2 == 0) {
          ...
          if (result != 0) {
            ESP_LOGE(TAG, "[%s] Failed to send VCSECInformationRequest",
                     current_command.execute_name.c_str());
          }
        }
        current_command.last_tx_at = now;
  ...
  case BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH_RESPONSE:
    if (now - current_command.last_tx_at > MAX_LATENCY) {
      ESP_LOGW(
          TAG,
          "[%s] Timeout while waiting for INFOTAINMENT SessionInfo, retrying..",
          current_command.execute_name.c_str());
      current_command.state = BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH;
      current_command.retry_count++;
    }
  ...
  case BLECommandState::READY:
    ...
    if (now - current_command.last_tx_at > MAX_LATENCY) {
      ...
      if (current_command.retry_count > MAX_RETRIES) {
        ESP_LOGE(TAG,
                 "[%s] Failed to execute command after %d retries, giving up",
                 current_command.execute_name.c_str(), MAX_RETRIES);
        command_queue_.pop();
      } else {
        ...
        if (result == 0) {
          ...
          if (strcmp(current_command.execute_name.c_str(), "wake vehicle") ==
              0) {
            current_command.state = BLECommandState::WAITING_FOR_WAKE_RESPONSE;
          } else {
...
void TeslaBLEVehicle::process_ble_write_queue() {
  ...
  esp_err_t err = esp_ble_gattc_write_char(
      gattc_if, conn_id, this->write_handle_, chunk.data.size(),
      chunk.data.data(), chunk.write_type, chunk.auth_req);
  if (err) {
    ESP_LOGW(TAG, "Error sending write value to BLE gattc server, error=%d",
             err);
  } else {
    ESP_LOGV(TAG, "BLE TX: %s",
...
void TeslaBLEVehicle::process_ble_read_queue() {
  ...
  ESP_LOGV(
      TAG, "BLE RX chunk: %s",
      format_hex(read_chunk_.buffer.data(), read_chunk_.buffer.size()).c_str());

  // check we are not overflowing the buffer before appending data
  size_t buffer_len_post_append =
      read_chunk_.buffer.size() + this->ble_read_buffer_.size();
  if (buffer_len_post_append > MAX_BLE_MESSAGE_SIZE) {
    ESP_LOGE(TAG, "BLE RX: Message length (%d) exceeds max BLE message size",
             buffer_len_post_append);
    // clear buffer
  ...
  this->ble_read_buffer_.insert(this->ble_read_buffer_.end(),
                                read_chunk_.buffer.begin(),
                                read_chunk_.buffer.end());
  this->ble_read_queue_.pop();

  if (this->ble_read_buffer_.size() >= 2) {
    int message_length =
        (this->ble_read_buffer_[0] << 8) | this->ble_read_buffer_[1];

    if (this->ble_read_buffer_.size() >= 2 + message_length) {
      ESP_LOGD(TAG, "BLE RX: %s",
               format_hex(this->ble_read_buffer_.data(),
                          this->ble_read_buffer_.size())
                   .c_str());
    } else {
      ESP_LOGD(TAG, "BLE RX: Buffered chunk, waiting for more data.. (%d/%d)",
               this->ble_read_buffer_.size(), 2 + message_length);
      return;
    }
  } else {
  ...
  UniversalMessage_RoutableMessage message =
      UniversalMessage_RoutableMessage_init_default;
  ...
  // clear read buffer
  this->ble_read_buffer_
      .clear(); // This will set the size to 0 and free unused memory
  this->ble_read_buffer_
      .shrink_to_fit(); // This will reduce the capacity to fit the size
...
void TeslaBLEVehicle::process_response_queue() {
  ...
  UniversalMessage_Domain domain =
      message.from_destination.sub_destination.domain;
  ...
  std::string request_uuid_hex_string =
      format_hex(message.request_uuid.bytes, message.request_uuid.size);
  const char *request_uuid_hex = request_uuid_hex_string.c_str();

  if (not message.has_to_destination) {
    ESP_LOGW(TAG, "[%s] Dropping message with missing destination",
             request_uuid_hex);
    return;
  }
  ...
  case UniversalMessage_Destination_domain_tag: {
    ESP_LOGD(TAG, "[%s] Dropping message to %s", request_uuid_hex,
             domain_to_string(domain));
    return;
  }
  ...
  default: {
    ESP_LOGW(TAG,
             "[%s] Dropping message with unrecognized destination type, %d",
             request_uuid_hex, message.to_destination.which_sub_destination);
    return;
  }
  }

  if (message.to_destination.sub_destination.routing_address.size != 16) {
    ESP_LOGW(TAG, "[%s] Dropping message with invalid address length",
             request_uuid_hex);
    return;
  }

  if (message.has_signedMessageStatus &&
      message.signedMessageStatus.operation_status ==
  ...
  if (message.which_payload ==
    ...
    ESP_LOGI(TAG, "[%s] Updated session info for %s", request_uuid_hex,
             domain_to_string(domain));
  }

  if (message.has_signedMessageStatus) {
    ESP_LOGD(TAG, "Received signed message status from domain %s",
             domain_to_string(domain));
    log_message_status(TAG, &message.signedMessageStatus);
    if (message.signedMessageStatus.operation_status ==
        UniversalMessage_OperationStatus_E_OPERATIONSTATUS_ERROR) {
      ESP_LOGE(TAG, "Received error message from domain %s",
               domain_to_string(domain));
      return;
    } else if (message.signedMessageStatus.operation_status ==
               UniversalMessage_OperationStatus_E_OPERATIONSTATUS_WAIT) {
      ESP_LOGI(TAG, "Received wait message from domain %s",
               domain_to_string(domain));
      return;
    } else {
      ESP_LOGI(TAG, "Received success message from domain %s",
               domain_to_string(domain));
    }
    return;
  }
  ...
  case UniversalMessage_Destination_domain_tag: {
    ESP_LOGD(TAG, "Received message from domain %s",
             domain_to_string(message.from_destination.sub_destination.domain));
    switch (message.from_destination.sub_destination.domain) {
    case UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY: {
      VCSEC_FromVCSECMessage vcsec_message =
          VCSEC_FromVCSECMessage_init_default;
      int return_code = tesla_ble_client_->parseFromVCSECMessage(
          &message.payload.protobuf_message_as_bytes, &vcsec_message);
      if (return_code != 0) {
      ...
      case VCSEC_FromVCSECMessage_vehicleStatus_tag: {
        ...
        if (!command_queue_.empty()) {
          ...
          case UniversalMessage_Domain_DOMAIN_INFOTAINMENT:
            ...
            case BLECommandState::WAITING_FOR_WAKE_RESPONSE:
              switch (
                  vcsec_message.sub_message.vehicleStatus.vehicleSleepStatus) {
              case VCSEC_VehicleSleepStatus_E_VEHICLE_SLEEP_STATUS_AWAKE:
                if (strcmp(current_command.execute_name.c_str(),
                           "wake vehicle") == 0) {
                  ESP_LOGI(TAG,
                           "[%s] Received vehicle status, command completed",
                           current_command.execute_name.c_str());
                  command_queue_.pop();
                } else {
                  ESP_LOGI(TAG,
                           "[%s] Received vehicle status, vehicle is awake",
                           current_command.execute_name.c_str());
                  current_command.state =
                      BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH;
                  current_command.retry_count = 0;
              ...
              default:
                ESP_LOGD(TAG,
                         "[%s] Received vehicle status, vehicle is not awake",
                         current_command.execute_name.c_str());
                break;
              }
              break;
            ...
            case BLECommandState::WAITING_FOR_RESPONSE:
              if (strcmp(current_command.execute_name.c_str(),
                         "wake vehicle") == 0 ||
                  strcmp(current_command.execute_name.c_str(), "data update") ==
                      0) {
                ESP_LOGI(TAG, "[%s] Received vehicle status, command completed",
                         current_command.execute_name.c_str());
                command_queue_.pop();
              } else if (strcmp(current_command.execute_name.c_str(),
                                "data update | forced") == 0) {
                switch (vcsec_message.sub_message.vehicleStatus
                            .vehicleSleepStatus) {
                case VCSEC_VehicleSleepStatus_E_VEHICLE_SLEEP_STATUS_AWAKE:
                  ESP_LOGI(TAG,
                           "[%s] Received vehicle status, command completed",
                ...
                default:
                  ESP_LOGD(
                      TAG,
                      "[%s] Received vehicle status, infotainment is not awake",
                      current_command.execute_name.c_str());
                  invalidateSession(
                      UniversalMessage_Domain_DOMAIN_INFOTAINMENT);
                  current_command.state =
          ...
          if (current_command.domain ==
            ...
            case VCSEC_OperationStatus_E_OPERATIONSTATUS_OK:
              if (current_command.state ==
                  BLECommandState::WAITING_FOR_RESPONSE) {
                ESP_LOGI(TAG,
                         "[%s] Received VCSEC OK message, command completed",
                         current_command.execute_name.c_str());
                command_queue_.pop();
              }
              break;
            case VCSEC_OperationStatus_E_OPERATIONSTATUS_WAIT:
              if (current_command.state ==
                ...
                ESP_LOGW(
                    TAG,
                    "[%s] Received VCSEC WAIT message, requeuing command..",
                    current_command.execute_name.c_str());
            ...
            case VCSEC_OperationStatus_E_OPERATIONSTATUS_ERROR:
              ESP_LOGW(TAG,
                       "[%s] Received VCSEC ERROR message, retrying command..",
                       current_command.execute_name.c_str());
              if (current_command.state ==
                  BLECommandState::WAITING_FOR_RESPONSE) {
                current_command.state = BLECommandState::READY;
      ...
      case VCSEC_FromVCSECMessage_nominalError_tag: {
        ...
        ESP_LOGE(TAG, "  error: %s",
                 generic_error_to_string(
                     vcsec_message.sub_message.nominalError.genericError));
        break;
      }
      default: {
        ...
        int return_code = tesla_ble_client_->parseVCSECInformationRequest(
            &message.payload.protobuf_message_as_bytes, &info_message);
        if (return_code != 0) {
        ...
        ESP_LOGD(TAG, "InformationRequest public key: %s",
                 format_hex(info_message.key.publicKey.bytes,
                            info_message.key.publicKey.size)
                     .c_str());
        return;
      } break;
      }
    ...
    case UniversalMessage_Domain_DOMAIN_INFOTAINMENT: {
      ...
      // New TeslaBLE::Client API requires signature context for potential
      // decryption
      Signatures_SignatureData *sig_data_ptr =
          &message.sub_sigData.signature_data;
      pb_size_t which_sig = message.which_sub_sigData;
      UniversalMessage_MessageFault_E fault =
          message.has_signedMessageStatus
      ...
      int return_code = tesla_ble_client_->parsePayloadCarServerResponse(
          &message.payload.protobuf_message_as_bytes, sig_data_ptr, which_sig,
          fault, &carserver_response);
      if (return_code != 0) {
        ESP_LOGE(TAG, "Failed to parse incoming message");
        return;
      }
      ...
      if (carserver_response.which_response_msg ==
          CarServer_Response_vehicleData_tag) {
        handleCarServerVehicleData(carserver_response.response_msg.vehicleData);
      }
        ...
        if (current_command.domain ==
          ...
          case CarServer_OperationStatus_E_OPERATIONSTATUS_OK:
            if (current_command.state ==
                BLECommandState::WAITING_FOR_RESPONSE) {
              ESP_LOGI(TAG,
                       "[%s] Received CarServer OK message, command completed",
                       current_command.execute_name.c_str());
              command_queue_.pop();
            }
            break;
          case CarServer_OperationStatus_E_OPERATIONSTATUS_ERROR:
            // if charging switch is turned on and reason = "is_charging" it's
            // OK if charging switch is turned of and reason = "is_not_charging"
            // it's OK
            if (carserver_response.actionStatus.has_result_reason) {
              switch (
                  carserver_response.actionStatus.result_reason.which_reason) {
              case CarServer_ResultReason_plain_text_tag:
                if (strcmp(carserver_response.actionStatus.result_reason.reason
                               .plain_text,
                           "is_charging") == 0 ||
                    strcmp(carserver_response.actionStatus.result_reason.reason
                               .plain_text,
                           "is_not_charging") == 0) {
                  ESP_LOGD(TAG, "[%s] Received charging status: %s",
                           current_command.execute_name.c_str(),
                           carserver_response.actionStatus.result_reason.reason
                               .plain_text);
                  if (current_command.state ==
                      BLECommandState::WAITING_FOR_RESPONSE) {
                    ESP_LOGI(
              ...
              ESP_LOGE(
                  TAG,
                  "[%s] Received CarServer ERROR message, retrying command..",
                  current_command.execute_name.c_str());
              if (current_command.state ==
                  BLECommandState::WAITING_FOR_RESPONSE) {
                current_command.state = BLECommandState::READY;
    ...
    default: {
      ESP_LOGD(TAG, "Received message for %s",
               domain_to_string(message.to_destination.sub_destination.domain));
      ESP_LOGD(
          TAG, "Received message from unknown domain %s",
          domain_to_string(message.from_destination.sub_destination.domain));
      break;
    } break;
    }
  ...
  default: {
    ESP_LOGD(TAG, "Received message from unknown domain %s",
             domain_to_string(message.from_destination.sub_destination.domain));
    break;
  } break;
  }
...
void TeslaBLEVehicle::loop() {
  if (this->node_state != espbt::ClientState::ESTABLISHED) {
    if (!command_queue_.empty()) {
      // clear command queue if not connected or on first boot (prevent restore
      // value triggering commands)
      command_queue_.pop();
    }
    return;
  }
...
void TeslaBLEVehicle::update() {
  ...
  if (this->node_state == espbt::ClientState::ESTABLISHED) {
    ...
    this->enqueueVCSECInformationRequest();

    uint32_t now = millis();
    bool is_awake = !this->isAsleepSensor->state;

    // Check for sleep -> awake transition
    if (this->was_asleep_ && is_awake) {
      ESP_LOGI(TAG, "Vehicle woke up, requesting initial charge data..");
      this->sendCarServerGetVehicleData();
      this->last_charge_poll_ = now;
      this->last_awake_poll_ = now;
    }
    this->was_asleep_ = !is_awake;

    // Poll for charge data when awake
    if (is_awake) {
      bool should_poll_charge = false;

      // Poll frequently when charging
      if (this->is_charging_ &&
          (now - this->last_charge_poll_) >= this->charge_polling_interval_) {
        ESP_LOGD(TAG, "Vehicle is charging, requesting charge data..");
        should_poll_charge = true;
      }
      // Poll less frequently when awake but not charging
      else if (!this->is_charging_ && (now - this->last_awake_poll_) >=
                                          this->awake_polling_interval_) {
        ESP_LOGD(TAG, "Vehicle is awake, requesting periodic charge data..");
        should_poll_charge = true;
      }

      if (should_poll_charge) {
        this->sendCarServerGetVehicleData();
        this->last_charge_poll_ = now;
        this->last_awake_poll_ = now;
      }
...
int TeslaBLEVehicle::nvs_save_session_info(
    const Signatures_SessionInfo &session_info,
    const UniversalMessage_Domain domain) {
  ESP_LOGD(TAG, "Storing updated session info in NVS for domain %s",
           domain_to_string(domain));
  const char *nvs_key = (domain == UniversalMessage_Domain_DOMAIN_INFOTAINMENT)
                            ? nvs_key_infotainment
                            : nvs_key_vcsec;

  // Estimate required buffer size
  size_t session_info_encode_buffer_size =
      Signatures_SessionInfo_size + 10; // Add some padding
  std::vector<pb_byte_t> session_info_encode_buffer(
      session_info_encode_buffer_size);

  // Encode session info into protobuf message
  int return_code = TeslaBLE::pb_encode_fields(
      session_info_encode_buffer.data(), &session_info_encode_buffer_size,
      Signatures_SessionInfo_fields, &session_info);
  if (return_code != 0) {
    ESP_LOGE(TAG, "Failed to encode session info for domain %s. Error code: %d",
             domain_to_string(domain), return_code);
    return return_code;
  }
  ESP_LOGD(TAG, "Session info encoded to %d bytes for domain %s",
           session_info_encode_buffer_size, domain_to_string(domain));
  ESP_LOGD(TAG, "Session info: %s",
           format_hex(session_info_encode_buffer.data(),
                      session_info_encode_buffer_size)
               .c_str());

  // Store encoded session info in NVS
  esp_err_t err = nvs_set_blob(this->storage_handle_, nvs_key,
                               session_info_encode_buffer.data(),
                               session_info_encode_buffer_size);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to set %s key in storage: %s",
             domain_to_string(domain), esp_err_to_name(err));
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to commit storage for domain %s: %s",
             domain_to_string(domain), esp_err_to_name(err));
    return static_cast<int>(err);
  }

  ESP_LOGD(TAG, "Successfully saved session info for domain %s",
           domain_to_string(domain));
  return 0;
}

int TeslaBLEVehicle::nvs_load_session_info(
    Signatures_SessionInfo *session_info,
    const UniversalMessage_Domain domain) {
  ...
  const std::string nvs_key =
      (domain == UniversalMessage_Domain_DOMAIN_INFOTAINMENT)
          ? nvs_key_infotainment
          : nvs_key_vcsec;

  size_t required_session_info_size = 0;
  esp_err_t err = nvs_get_blob(this->storage_handle_, nvs_key.c_str(), nullptr,
                               &required_session_info_size);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to read %s key size from storage: %s",
             domain_to_string(domain), esp_err_to_name(err));
    return 1;
  }

  std::vector<uint8_t> session_info_protobuf(required_session_info_size);
  err = nvs_get_blob(this->storage_handle_, nvs_key.c_str(),
                     session_info_protobuf.data(), &required_session_info_size);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to read %s key data from storage: %s",
             domain_to_string(domain), esp_err_to_name(err));
    return 1;
  }
  ...
  ESP_LOGD(TAG, "Session info: %s",
           format_hex(session_info_protobuf.data(), required_session_info_size)
               .c_str());

  pb_istream_t stream = pb_istream_from_buffer(session_info_protobuf.data(),
                                               required_session_info_size);
  if (!pb_decode(&stream, Signatures_SessionInfo_fields, session_info)) {
    ESP_LOGE(TAG, "Failed to decode session info response: %s",
             PB_GET_ERROR(&stream));
    return 1;
  }
...
int TeslaBLEVehicle::nvs_initialize_private_key() {
  ...
  if (required_private_key_size == 0) {
    ...
    tesla_ble_client_->getPrivateKey(
        private_key_buffer, sizeof(private_key_buffer), &private_key_length);
...
void TeslaBLEVehicle::regenerateKey() {
  ...
  err = nvs_set_blob(this->storage_handle_, "private_key", private_key_buffer,
                     private_key_length);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed commit storage: %s", esp_err_to_name(err));
  }
...
int TeslaBLEVehicle::startPair() {
  ...
  // support for wake command will be added to ROLE_CHARGING_MANAGER in a future
  ...
  int return_code = tesla_ble_client_->buildWhiteListMessage(
      Keys_Role_ROLE_DRIVER, VCSEC_KeyFormFactor_KEY_FORM_FACTOR_CLOUD_KEY,
  ...
  return_code = writeBLE(whitelist_message_buffer, whitelist_message_length,
...
int TeslaBLEVehicle::sendSessionInfoRequest(UniversalMessage_Domain domain) {
  ...
  int return_code = tesla_ble_client_->buildSessionInfoRequestMessage(
      domain, message_buffer, &message_length);
  ...
  return_code = writeBLE(message_buffer, message_length,
                         ESP_GATT_WRITE_TYPE_NO_RSP, ESP_GATT_AUTH_REQ_NONE);
  if (return_code != 0) {
...
int TeslaBLEVehicle::writeBLE(const unsigned char *message_buffer,
                              size_t message_length,
                              esp_gatt_write_type_t write_type,
                              esp_gatt_auth_req_t auth_req) {
  ESP_LOGD(TAG, "BLE TX: %s",
           format_hex(message_buffer, message_length).c_str());
  // BLE MTU is 23 bytes, so we need to split the message into chunks (20 bytes
  // as in vehicle_command)
  for (size_t i = 0; i < message_length; i += BLOCK_LENGTH) {
    size_t chunkLength =
        std::min(static_cast<size_t>(BLOCK_LENGTH), message_length - i);
    std::vector<unsigned char> chunk(message_buffer + i,
                                     message_buffer + i + chunkLength);
...
int TeslaBLEVehicle::sendVCSECActionMessage(VCSEC_RKEAction_E action) {
  ...
  int return_code = tesla_ble_client_->buildVCSECActionMessage(
  ...
  return_code = writeBLE(action_message_buffer, action_message_buffer_length,
                         ESP_GATT_WRITE_TYPE_NO_RSP, ESP_GATT_AUTH_REQ_NONE);
  if (return_code != 0) {
...
int TeslaBLEVehicle::wakeVehicle() {
  ...
  command_queue_.emplace(
      UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY,
      [this]() {
        int return_code = this->sendVCSECActionMessage(
            VCSEC_RKEAction_E_RKE_ACTION_WAKE_VEHICLE);
        if (return_code != 0) {
...
void TeslaBLEVehicle::enqueueVCSECInformationRequest(bool force) {
  ...
  command_queue_.emplace(
      force ? UniversalMessage_Domain_DOMAIN_INFOTAINMENT
            : UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY,
      [this]() {
  ...
  // When forced, also request charge data if vehicle is awake
  if (force && !this->isAsleepSensor->state) {
    ESP_LOGD(TAG, "Force update: also requesting charge data");
    this->sendCarServerGetVehicleData();
    uint32_t now = millis();
    this->last_charge_poll_ = now;
    this->last_awake_poll_ = now;
...
int TeslaBLEVehicle::sendCarServerGetVehicleData() {
  ...
  command_queue_.emplace(
      ...
      [this]() {
        ...
        size_t message_length = 0;
        int return_code =
            tesla_ble_client_->buildCarServerGetVehicleDataMessage(
                message_buffer, &message_length,
        ...
        return_code =
            writeBLE(message_buffer, message_length, ESP_GATT_WRITE_TYPE_NO_RSP,
...
int TeslaBLEVehicle::sendCarServerVehicleActionMessage(
    BLE_CarServer_VehicleAction action, int param) {
  ...
  ESP_LOGI(TAG, "[%s] Adding command to queue (param=%d)", action_str.c_str(),
           static_cast<int>(param));
  command_queue_.emplace(
      UniversalMessage_Domain_DOMAIN_INFOTAINMENT,
      [this, action, action_str, param]() {
        unsigned char message_buffer[UniversalMessage_RoutableMessage_size];
        size_t message_length = 0;
        int return_code = 0;
...
int TeslaBLEVehicle::handleSessionInfoUpdate(
    UniversalMessage_RoutableMessage message, UniversalMessage_Domain domain) {
  ESP_LOGD(TAG, "Received session info response from domain %s",
           domain_to_string(domain));

  auto session = tesla_ble_client_->getPeer(domain);

  // parse session info
  UniversalMessage_RoutableMessage_session_info_t sessionInfo =
      message.payload.session_info;
  Signatures_SessionInfo session_info = Signatures_SessionInfo_init_default;
  int return_code = tesla_ble_client_->parsePayloadSessionInfo(
      &message.payload.session_info, &session_info);
  if (return_code != 0) {
  ...
  if (return_code != 0) {
    ESP_LOGE(TAG, "Failed to save %s session info to NVS",
             domain_to_string(domain));
  }
...
void TeslaBLEVehicle::invalidateSession(UniversalMessage_Domain domain) {
  ...
  if (!command_queue_.empty()) {
    ...
    case UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY:
      if (domain == UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY) {
        ESP_LOGW(TAG,
                 "[%s] VCSEC session invalid, requesting new session info..",
                 current_command.execute_name.c_str());
        current_command.state = BLECommandState::WAITING_FOR_VCSEC_AUTH;
      }
      break;
    case UniversalMessage_Domain_DOMAIN_INFOTAINMENT:
      if (domain == UniversalMessage_Domain_DOMAIN_INFOTAINMENT) {
        ESP_LOGW(
            TAG,
            "[%s] INFOTAINMENT session invalid, requesting new session info..",
            current_command.execute_name.c_str());
        current_command.state = BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH;
      }
      break;
...
int TeslaBLEVehicle::handleVCSECVehicleStatus(
  ...
  if (vehicleStatus.vehicleSleepStatus ==
...
void TeslaBLEVehicle::gattc_event_handler(esp_gattc_cb_event_t event,
  ...
  case ESP_GATTC_SRVC_CHG_EVT: {
    ...
    ESP_LOGD(TAG, "ESP_GATTC_SRVC_CHG_EVT, bd_addr: %s",
             format_hex(bda, sizeof(esp_bd_addr_t)).c_str());
    break;
  }
  ...
  case ESP_GATTS_READ_EVT: {
    ...
    if (param->read.status != ESP_GATT_OK) {
      ESP_LOGW(TAG, "Error reading char at handle %d, status=%d",
  ...
  case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
    if (param->reg_for_notify.status != ESP_GATT_OK) {
      ESP_LOGE(TAG, "reg for notify failed, error status = %x",
    ...
    unsigned char private_key_buffer[PRIVATE_KEY_SIZE];
    size_t private_key_length = 0;
    int return_code = tesla_ble_client_->getPrivateKey(
        private_key_buffer, sizeof(private_key_buffer), &private_key_length);
    if (return_code != 0) {
  ...
  case ESP_GATTC_WRITE_DESCR_EVT: {
    ...
    if (param->write.status != ESP_GATT_OK) {
      ESP_LOGW(TAG, "Error writing descriptor at handle %d, status=%d",
...
int TeslaBLEVehicle::handleCarServerVehicleData(
  ...
  if (vehicleData.has_charge_state) {
    ...
    if (charge_state.which_optional_battery_level) {
      this->updateBatteryLevel(static_cast<float>(
          charge_state.optional_battery_level.battery_level));
    }

    // Update charging state and determine if charging
    if (charge_state.has_charging_state) {
      ...
      this->updateChargingState(stateStr);
      this->is_charging_ = currently_charging;
      ESP_LOGD(TAG, "Charging state: %s, is_charging: %s", stateStr,
