...
void MessageHandler::handle_universal_message(const UniversalMessage_RoutableMessage& message) {
    ...
    if (message.has_signedMessageStatus) {
        ...
        TeslaBLE::log_message_status(MESSAGE_HANDLER_TAG, &message.signedMessageStatus);
            ...
            switch (message.from_destination.sub_destination.domain) {
                ...
                default:
                    ESP_LOGD(MESSAGE_HANDLER_TAG, "Message from unknown domain: %s", 
                             TeslaBLE::domain_to_string(message.from_destination.sub_destination.domain));
                    break;
            }
            break;
...
void MessageHandler::handle_vcsec_message(const UniversalMessage_RoutableMessage& message) {
    ...
    switch (vcsec_message.which_sub_message) {
        ...
        case VCSEC_FromVCSECMessage_commandStatus_tag:
            ...
            TeslaBLE::log_vcsec_command_status(MESSAGE_HANDLER_TAG, &vcsec_message.sub_message.commandStatus);
            update_command_state_on_response(message);
            break;
        ...
        case VCSEC_FromVCSECMessage_nominalError_tag:
            ESP_LOGE(MESSAGE_HANDLER_TAG, "Received nominal error: %s", 
                     TeslaBLE::generic_error_to_string(vcsec_message.sub_message.nominalError.genericError));
            break;
...
void MessageHandler::handle_carserver_message(const UniversalMessage_RoutableMessage& message) {
    ...
    if (fault != UniversalMessage_MessageFault_E_MESSAGEFAULT_ERROR_NONE) {
        ESP_LOGW(MESSAGE_HANDLER_TAG, "Message fault detected: %s", TeslaBLE::message_fault_to_string(fault));
    }
    ...
    TeslaBLE::log_carserver_response(MESSAGE_HANDLER_TAG, &carserver_response);
...
void MessageHandler::handle_session_info(const UniversalMessage_RoutableMessage& message, UniversalMessage_Domain domain) {
    ESP_LOGD(MESSAGE_HANDLER_TAG, "Handling session info for %s", TeslaBLE::domain_to_string(domain));
    ...
    if (result != 0) {
        ESP_LOGE(MESSAGE_HANDLER_TAG, "Failed to parse session info for %s: %d", TeslaBLE::domain_to_string(domain), result);
    ...
    if (update_result == 0) {
        ESP_LOGI(MESSAGE_HANDLER_TAG, "Updated session info for %s", TeslaBLE::domain_to_string(domain));
        ...
        ESP_LOGE(MESSAGE_HANDLER_TAG, "Failed to update session info for %s: %d", TeslaBLE::domain_to_string(domain), update_result);
...
void MessageHandler::handle_vehicle_status(const VCSEC_VehicleStatus& status) {
    TeslaBLE::log_vehicle_status(MESSAGE_HANDLER_TAG, &status);
